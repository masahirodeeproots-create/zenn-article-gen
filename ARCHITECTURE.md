...
[Phase 0: 知識の準備] ──────────────────────────────────────
    ...
    │      → 7日超 or なし → 検索実行
    │
    │    Step 3: メディアごとに検索（エージェントがWebFetchで実行）
    │      ┌──────────────────┬─────────────────┬──────────────┬───────────────────┐
    │      │ メディア           │ 収集方法          │ 人気指標      │ 取得するもの         │
    │      ├──────────────────┼─────────────────┼──────────────┼───────────────────┤
    │      │ Qiita             │ 公式API v2       │ likes/stocks │ 人気順上位20件      │
    │      │ Hacker News       │ 公式API          │ score        │ ポイント上位20件     │
    │      │ Zenn              │ 非公式JSON       │ liked/bookmarks │ 人気順上位20件   │
    │      │ OpenAI Blog       │ RSS→sitemap→本文 │ なし(自前判定) │ 関連記事           │
    │      │ Anthropic Blog    │ sitemap→本文     │ なし(自前判定) │ 関連記事           │
    │      └──────────────────┴─────────────────┴──────────────┴───────────────────┘
    │
    │      実行方法:
    │        API組（Qiita/HN/Zenn）→ エージェントがWebFetchでAPIを叩く
    │        Blog組（OpenAI/Anthropic）→ エージェントがWebFetchでsitemap取得→記事URL抽出→本文取得
    │        WebSearchはフォールバック（上記で取れなかったときのみ）
    │
    │    Step 4: 結果を整理
...
| メディア | 収集方法 | 人気指標 | 言語 | 取得するもの |
|---------|---------|---------|------|------------|
| Qiita | 公式API v2 | likes / comments / stocks | 日本語 | 人気記事。トレンド+痛み |
| Hacker News | 公式API | score / descendants | 英語 | 海外トレンド、議論 |
| Zenn | 非公式JSON | liked / comments / bookmarks | 日本語 | 人気記事。トレンド+痛み |
| OpenAI Blog | RSS → sitemap → 本文 | なし（自前スコア） | 英語 | 公式情報 |
| Anthropic Blog | sitemap → 記事ページ | なし（自前スコア） | 英語 | 公式見解、ベストプラクティス |

エージェントがWebFetchで全て実行する。
API組（Qiita/HN/Zenn）は人気順で直接取得できる。
Blog組（OpenAI/Anthropic）は人気指標がないため、エージェントが記事本文を読んでお題との関連度で判定する。
WebSearchは上記で取れなかったときのフォールバック。
