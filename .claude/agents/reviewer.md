...
### 読者の学び
- [ ] この記事から読者が学べることを1文で言えるか？（言えないなら記事の目的がない）
- [ ] 「自分のプロジェクトでも使える」と思えるポイントがあるか？
- [ ] 専門用語に初出時の説明があるか？（専門家でない読者も読めるか）

### コード比率
- [ ] コード比率が20%を超えていないか？（超えている → 「コードで説明を省略している。言葉で語れ」と指摘）

### アーキテクチャ図
- [ ] 全体構造がわかるアーキテクチャ図（ASCII図）が最低1つあるか？

### スコアルール
**AI臭検出で2つ以上の項目に該当した場合、Authenticity は最大 6/10 とする。**
**Authenticity が 8/10 未満の場合、Overall は 8.0 を超えてはならない。**
**読者価値チェックで「読者の学びが1文で言えない」場合、Overall は 7.5 を超えてはならない。**
...
### Comparison with Human Benchmarks（🔴 必須・最低2本と段落レベルで比較）

以下の形式で、サンプリングされた5本のうち**最低2本**と具体的に比較すること:

```
ベンチマーク記事「{タイトル}」の{箇所}:
  "{実際のテキストを引用}"
  → {何が良いか: 具体的な数字、自嘲、文脈、感情、雑さ等}
...
**2本以上でこの段落レベルの比較を行い、差分が大きい場合はAuthenticityを下げること。**
